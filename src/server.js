...
app.get('/api/datasets/:id/rows', authRequired, activeDataset, (req, res) => {
  ...
  const whereSql = where.join(' AND '); const sortMap = { similarity: 'dr.similarity_score', confidence: 'dr.ai_confidence', row: 'dr.row_number', report_time: 'dr.row_number' }; const order = sortMap[clean(q.sort)] || 'dr.row_number'; const direction = String(q.direction).toLowerCase() === 'desc' ? 'DESC' : 'ASC';
...
app.get('/api/datasets/:id/facets', authRequired, activeDataset, (req, res) => {
  ...
  res.json({ ai_judgment: distinct('ai_judgment'), human_judgment: distinct('human_judgment'), business_group: distinct('main_business_group'), ai_route: distinct('ai_route') });
});
